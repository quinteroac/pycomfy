...
def run(
    *,
    ...
    fps:
        Target frame rate of the generated video (default 24).  This value is
        currently reserved — :func:`~comfy_diffusion.latent.ltxv_empty_latent_video`
        does not yet expose an fps argument.  The parameter is accepted so callers
        can document intent; it will be forwarded automatically once upstream
        support is added.
    steps:
