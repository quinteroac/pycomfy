...
def manifest() -> list[ModelEntry]:
    ...
    also required, matching the reference ``video_ltx2_3_i2v.json`` workflow
    exactly.
...
def run(
    *,
    ...
    Mirrors the ``video_ltx2_3_i2v`` official workflow.  Two-pass sampling with
    spatial upscaling: the input image is injected at strength ``guide_strength_pass1``
    before pass 1 and re-injected at strength ``guide_strength_pass2`` into the
    upscaled latent before pass 2.
    ...
    guide_strength_pass2 : float, optional
        ``LTXVImgToVideoInplace`` strength on the upscaled latent before pass 2.
        Default ``1.0``.
    ...
    # Create empty video latent and inject image guide at pass-1 strength.
    video_latent = ltxv_empty_latent_video(width=width, height=height, length=length)
    video_latent = ltxv_img_to_video_inplace(vae, preprocessed, video_latent, strength=guide_strength_pass1)

    # LTXVCropGuides — strips keyframe metadata from conditioning (pass-through
    # when no keyframe_idxs are present, which is the case with LTXVImgToVideoInplace).
    positive, negative, _ = ltxv_crop_guides(positive, negative, video_latent)
    ...
    # Re-inject image into the upscaled latent at pass-2 strength.
    video_upscaled = ltxv_img_to_video_inplace(vae, preprocessed, video_upscaled, strength=guide_strength_pass2)
