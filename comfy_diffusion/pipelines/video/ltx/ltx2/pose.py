"""LTX-Video 2 Pose-to-Video pipeline (dev fp8 checkpoint, audio-visual).
...
- ``run()`` executes the full inference pipeline end-to-end: video loading,
  frame extraction, DWPreprocessor pose estimation, model loading, LoRA
  application, conditioning with first-frame guide, AV sampling, latent
  upsampling, and decoding.

The Pose-to-Video variant uses:

- The dev fp8 checkpoint (``ltx-2-19b-dev-fp8.safetensors``) loaded via
  ``CheckpointLoaderSimple`` from ``checkpoints/``.
- A pose-control LoRA (``ltx-2-19b-ic-lora-pose-control.safetensors``)
...
- DWPreprocessor pose estimation (detect_body=True, detect_hand=True,
  detect_face=False, resolution=512) applied to video frames extracted from
  the input video.
- A first-frame guide injected via
  :func:`~comfy_diffusion.video.ltxv_img_to_video_inplace` into the
  half-resolution latent and again at full resolution before pass 2.  The
  first frame is loaded from a separate image path (``first_frame_path``)
  rather than extracted from the video.
- The pose frames (full resolution) are injected as the spatial guide via
  :func:`~comfy_diffusion.conditioning.ltxv_add_guide`.
- A spatial latent upsampler (``ltx-2-spatial-upscaler-x2-1.0.safetensors``)
  applied between passes.
- The full audio-visual (AV) sampling chain — video and audio generated
  together.
...
_HF_REPO_POSE = "Lightricks/LTX-2-19b-IC-LoRA-Pose-Control"
...
_POSE_LORA_DEST = Path("loras") / "ltx-2-19b-ic-lora-pose-control.safetensors"
...
# DWPreprocessor parameters (matches workflow DWPreprocessor node).
_DW_RESOLUTION = 512
...
def manifest() -> list[ModelEntry]:
    """Return the list of model files required by the LTX-Video 2 Pose-to-Video pipeline.
...
def run(
    *,
    ...
    pose_lora_filename: str | None = None,
    lora_filename: str | None = None,
    upscaler_filename: str | None = None,
) -> dict[str, Any]:
    """Run the LTX-Video 2 Pose-to-Video pipeline end-to-end.

    Two-pass sampling mirroring the ``video_ltx2_pose_to_video.json`` reference
    workflow (LTX 2.0 dev fp8 subgraph):

    - Pass 1: base model + pose-control LoRA at half resolution using
      ``LTXVScheduler`` (20 steps) and ``euler``; CFG=3.
    - Pass 2: pose LoRA + distilled LoRA at full resolution using
    ...
    video_path : str | Path
        Path to the input video file.  Frames are extracted and passed through
        DWPreprocessor pose estimation to generate the structure-control signal
        for pass 1.
    prompt : str
    ...
    first_frame_path : str | Path | None, optional
        Path to a separate first-frame image used as the temporal anchor for
        both passes.  When ``None`` the first frame is extracted from the input
        video instead.  Default ``None``.
    width : int, optional
    ...
    cfg_pass1 : float, optional
        CFG scale for pass 1 (pose LoRA, half resolution).  Default ``3.0``.
    cfg_pass2 : float, optional
        CFG scale for pass 2 (pose + distilled LoRA, full resolution).
        Default ``1.0``.
    seed : int, optional
        Random seed.  Both passes use ``seed``.  Default ``0``.
    pose_lora_strength : float, optional
        Strength of the pose-control LoRA.  Default ``1.0``.
    ...
    pose_lora_filename : str | None, optional
        Override the default pose-control LoRA filename.  Default ``None``.
    ...
    from comfy_diffusion.image import dw_preprocessor, image_from_batch, image_scale_by, load_image, ltxv_preprocess
    ...
    from comfy_diffusion.video import get_video_components, load_video, ltxv_img_to_video_inplace
    ...
    pose_lora_path = (
        Path(pose_lora_filename) if pose_lora_filename else models_dir / _POSE_LORA_DEST
    )
    ...
    # Apply pose-control LoRA — model-only (LoraLoaderModelOnly in workflow).
    model_pose, _ = apply_lora(model_base, clip, pose_lora_path, pose_lora_strength, 0.0)

    # Extract video frames and run DWPreprocessor pose estimation.
    video = load_video(video_path)
    frames_raw, _ = get_video_components(video)
    frames = image_from_batch(frames_raw, 0, length)
    pose_frames = dw_preprocessor(
        frames,
        detect_body=True,
        detect_hand=True,
        detect_face=False,
        resolution=_DW_RESOLUTION,
    )
    # Resize pose frames to full output resolution, scale to half for pass 1.
    pose_frames_full = ltxv_preprocess(pose_frames, width, height)
    pose_frames_half = image_scale_by(pose_frames_full, "lanczos", 0.5)

    # Load first frame: from first_frame_path if provided, otherwise extract
    # from the video at full resolution.
    if first_frame_path is not None:
        first_frame_img, _ = load_image(first_frame_path)
        first_frame_full = ltxv_preprocess(first_frame_img, width, height)
    else:
        frames_full = ltxv_preprocess(frames, width, height)
        first_frame_full = image_from_batch(frames_full, 0, 1)
    ...
    # Pass 1: half resolution, pose LoRA, LTXVScheduler (20 steps) + euler.
    ...
    video_latent = ltxv_img_to_video_inplace(vae, first_frame_full, video_latent)
    # Add pose frames as spatial guide (updates conditioning and latent).
    positive, negative, video_latent = ltxv_add_guide(
        positive, negative, vae, video_latent, pose_frames_full
    )
    ...
    guider_p1 = cfg_guider(model_pose, positive, negative, cfg_pass1)
    ...
    video_up = ltxv_img_to_video_inplace(vae, first_frame_full, video_up)
    av_latent_p2 = ltxv_concat_av_latent(video_up, audio_latent_p1)

    # Pass 2: pose LoRA + distilled LoRA, ManualSigmas (4 steps) + gradient_estimation.
    model_p2, clip = apply_lora(model_pose, clip, lora_path, lora_strength, 0.0)
