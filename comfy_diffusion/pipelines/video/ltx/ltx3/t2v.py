"""LTX-Video 2.3 (22B distilled) text-to-video pipeline.
...
- ``run()`` executes the full inference pipeline end-to-end: model loading,
  conditioning, sampling, and AV latent decoding.

This pipeline mirrors ``comfyui_official_workflows/video/ltx/ltx3/`` and uses
the LTX-Video 2.3 AV (audio-visual) model, which generates video **and** audio
simultaneously.  The full sampling chain follows the ``video_ltx2_3_flf2v``
reference workflow:

- ``LTXVConditioning`` injects frame-rate metadata.
- Video and audio latents are concatenated with ``LTXVConcatAVLatent`` before
  sampling.
- ``CFGGuider`` + ``SamplerCustomAdvanced`` + ``ManualSigmas`` drive the
  denoising loop.
- After sampling, ``LTXVSeparateAVLatent`` splits the AV result.
- Video is decoded with ``VAEDecodeTiled``; audio with ``LTXVAudioVAEDecode``.

The audio VAE and video VAE are both loaded from the same checkpoint
(``ltx-2.3-22b-distilled-fp8.safetensors``) so no extra model download is
needed.
...
def run(
    *,
    ...
    """Run the LTX-Video 2.3 (22B distilled) text-to-video-with-audio pipeline.

    Mirrors the ``video_ltx2_3_t2v`` official workflow.  The LTX 2.3 model is
    audio-visual: it generates video frames **and** an audio track in a single
    sampling pass.
    ...
    prompt : str
        Positive text prompt.  Describe both visual content and audio in the
        same prompt (e.g. ``"a pianist playing jazz, warm studio sound"``).
    ...
    unet_filename : str | None, optional
        Override the default UNet filename.  When ``None`` the path from
        :func:`manifest` is used.  Default ``None``.
    ...
    # Build the sampling chain (mirrors the flf2v reference workflow).
