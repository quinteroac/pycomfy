"""WAN 2.2 first-last-frame-to-video pipeline (14B dual-model).
...
- ``run()`` executes the full inference pipeline end-to-end: model loading,
  ``WanFirstLastFrameToVideo`` conditioning, dual two-pass ``KSamplerAdvanced``
  with ``ModelSamplingSD3`` (shift=8) patched on both models.

This pipeline mirrors the ``video_wan2_2_14B_flf2v.json`` reference workflow.
It reuses the I2V 14B dual UNet weights with no LoRA (all LoRA nodes are
bypassed in the reference workflow).

Workflow data flow
------------------
1. Load high-noise UNet
2. Load low-noise UNet
3. Load text encoder (UMT5-XXL) + VAE (WAN 2.1)
4. Apply ModelSamplingSD3(shift=8) to both UNets
5. Encode positive and negative text prompts
6. Load start and end images; convert to BHWC float32 tensors
7. WanFirstLastFrameToVideo — builds FLF2V conditioning and initial latent
   (clip_vision inputs are unconnected in the reference workflow)
8. KSamplerAdvanced pass 1 — high-noise model: add_noise=True,
   start_at_step=0, end_at_step=steps//2, return_with_leftover_noise=True
9. KSamplerAdvanced pass 2 — low-noise model: add_noise=False,
   start_at_step=steps//2, end_at_step=steps, return_with_leftover_noise=False
10. VAEDecode → PIL frames
...
def manifest() -> list[ModelEntry]:
    """Return the list of model files required by the WAN 2.2 FLF2V pipeline.

    Returns exactly 4 :class:`~comfy_diffusion.downloader.HFModelEntry` items:
    two UNets (high-noise and low-noise), one text encoder (UMT5-XXL), and one
    VAE (WAN 2.1).  No LoRA entries are included — all LoRA nodes are bypassed
...
def run(
    ...
    *,
    ...
    """Run the WAN 2.2 first-last-frame-to-video pipeline end-to-end.

    Dual two-pass ``KSamplerAdvanced`` flow mirroring the
    ``video_wan2_2_14B_flf2v.json`` reference workflow.  The high-noise UNet
    handles pass 1 (steps 0→steps//2) and the low-noise UNet handles pass 2
    (steps//2→steps), each with ``ModelSamplingSD3`` (shift=8) applied.

    Parameters
    ----------
    start_image : PIL.Image.Image
        First frame of the interpolation.  Converted to a float32 BHWC tensor
        internally before passing to ``WanFirstLastFrameToVideo`` conditioning.
    end_image : PIL.Image.Image
        Last frame of the interpolation.  Converted to a float32 BHWC tensor
        internally before passing to ``WanFirstLastFrameToVideo`` conditioning.
    prompt : str
    ...
    # Convert PIL images to BHWC float32 tensors for WanFirstLastFrameToVideo.
    start_tensor = image_to_tensor(start_image)
    end_tensor = image_to_tensor(end_image)

    # WanFirstLastFrameToVideo — builds FLF2V conditioning and initial latent.
    # clip_vision inputs are unconnected in the reference workflow.
