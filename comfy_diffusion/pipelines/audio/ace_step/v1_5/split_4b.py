"""ACE Step 1.5 split text-to-audio pipeline — 4B text encoder variant.
...
- ``run()`` executes the full inference pipeline end-to-end: separate UNet,
  CLIP (0.6B + 4B text encoders), and VAE loading, followed by AuraFlow
  model-sampling patch, latent creation, text conditioning, KSampler
  denoising, and VAE audio decoding.

This pipeline mirrors the ``audio_ace_step_1_5_split_4b.json`` reference
...
checkpoint.  The 4B text encoder provides higher-capacity conditioning
compared to the 1.7B variant.
...
::
    ...
    from comfy_diffusion.pipelines.audio.ace_step.v1_5.split_4b import manifest, run
...
_CLIP_4B_DEST = Path("text_encoders") / "qwen_4b_ace15.safetensors"
...
def manifest() -> list[ModelEntry]:
    """Return the list of model files required by the ACE Step 1.5 split pipeline (4B variant).

    Returns four :class:`~comfy_diffusion.downloader.HFModelEntry` items — the
    diffusion UNet, the 0.6B CLIP text encoder, the 4B CLIP text encoder,
    ...
    return [
        ...
        HFModelEntry(
            repo_id=_HF_REPO,
            filename="text_encoders/qwen_4b_ace15.safetensors",
            dest=_CLIP_4B_DEST,
        ),
        HFModelEntry(
            repo_id=_HF_REPO,
...
def run(
    *,
    ...
    clip_4b_filename: str | None = None,
    vae_filename: str | None = None,
) -> dict[str, Any]:
    """Run the ACE Step 1.5 split text-to-audio pipeline (4B text encoder).

    Mirrors the ``audio_ace_step_1_5_split_4b.json`` reference workflow:
    ...
    clip_4b_filename : str | None, optional
        Override the default 4B CLIP filename.  When ``None``, uses
        ``qwen_4b_ace15.safetensors`` from the manifest.  Default ``None``.
    ...
    entries = manifest()
    unet_dest, clip_0_6b_dest, clip_4b_dest, vae_dest = (e.dest for e in entries)
    ...
    clip_4b_path = clip_4b_filename or clip_4b_dest.name
    ...
    clip = mm.load_clip(clip_0_6b_path, clip_4b_path, clip_type="ace")
