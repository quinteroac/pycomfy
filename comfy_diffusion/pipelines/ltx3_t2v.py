...
Key differences from ``ltx2_t2v_distilled``:

- Uses the 22B distilled LTX 2.3 checkpoint
  (``ltx-2.3-22b-distilled-fp8.safetensors``) for higher-quality output.
- Same Gemma 3 12B text encoder loaded via
  :meth:`~comfy_diffusion.models.ModelManager.load_ltxav_text_encoder`.
- Same spatial latent upsampler applied after sampling and before VAE decoding.
- Default ``steps=8`` (distilled model).
...
def run(
    *,
    ...
    """Run the LTX-Video 2.3 (22B distilled) text-to-video pipeline end-to-end.
