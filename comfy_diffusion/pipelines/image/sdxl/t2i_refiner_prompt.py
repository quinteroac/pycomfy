"""SDXL base + refiner text-to-image pipeline with separate prompts per stage.
...
This pipeline mirrors the standard SDXL base + refiner workflow (identical to
``t2i.py``) but allows separate prompts for the base and refiner stages.  The
base stage encodes ``prompt``/``negative_prompt`` with the base CLIP encoder,
while the refiner stage encodes ``refiner_prompt``/``refiner_negative_prompt``
with the refiner CLIP encoder.  Both default to the base prompt values when not
provided.

Pass 1 runs ``KSamplerAdvanced`` on the base model with leftover noise enabled,
stopping at ``base_end_step``; pass 2 continues denoising with the refiner model
starting from where pass 1 stopped.  The final latent is decoded with the
refiner VAE.
...
::
    ...
    # 2. Run inference with separate prompts for base and refiner.
    images = run(
        models_dir="/path/to/models",
        prompt="a majestic eagle soaring over snow-capped mountains, wide shot",
        refiner_prompt="photorealistic, sharp focus, award-winning photography",
    )
...
def manifest() -> list[ModelEntry]:
    """Return the list of model files required by this pipeline.
...
def run(
    *,
    ...
    refiner_negative_prompt: str | None = None,
    width: int = 1024,
    height: int = 1024,
    steps: int = 25,
    ...
    """Run the SDXL base + refiner T2I pipeline with separate prompts per stage.
    ...
    prompt : str
        Positive text prompt for the base stage.
    negative_prompt : str, optional
        Negative text prompt for the base stage.  Default ``""``.
    refiner_prompt : str | None, optional
        Positive text prompt for the refiner stage.  Defaults to ``prompt``
        when ``None``.
    refiner_negative_prompt : str | None, optional
        Negative text prompt for the refiner stage.  Defaults to
        ``negative_prompt`` when ``None``.
    width : int, optional
    ...
    # Resolve per-stage prompts — fall back to base prompt when not provided.
    effective_refiner_prompt = refiner_prompt if refiner_prompt is not None else prompt
    effective_refiner_negative = (
        refiner_negative_prompt if refiner_negative_prompt is not None else negative_prompt
    )
    ...
    # Encode prompts: base stage uses base CLIP, refiner stage uses refiner CLIP.
    base_positive, base_negative = encode_prompt(base_clip, prompt, negative_prompt)
    refiner_positive, refiner_negative = encode_prompt(
        refiner_clip, effective_refiner_prompt, effective_refiner_negative
    )
