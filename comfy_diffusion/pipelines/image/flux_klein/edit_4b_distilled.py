"""Flux.2 Klein 4B distilled image-edit pipeline.
...
This pipeline mirrors the *Image Edit (Flux.2 Klein 4B Distilled)* subgraph in
...
Flux2Scheduler → CFGGuider (cfg=1) → SamplerCustomAdvanced → VAEDecode``

The distilled variant uses ``cfg=1`` and derives the negative conditioning via
``conditioning_zero_out`` applied to the positive conditioning, requiring only
4 steps for high-quality results.
...
::
    ...
    from comfy_diffusion.pipelines.image.flux_klein.edit_4b_distilled import manifest, run
...
_HF_REPO_CLIP_4B = "Comfy-Org/z_image_turbo"
...
_UNET_DEST = Path("diffusion_models") / "flux-2-klein-4b-fp8.safetensors"
...
def manifest() -> list[ModelEntry]:
    """Return the list of model files required by the Flux.2 Klein 4B distilled edit pipeline.

    Returns exactly 3 model entries:

    - ``diffusion_models/flux-2-klein-4b-fp8.safetensors`` — UNet (distilled)
...
def run(
    *,
    ...
    """Run the Flux.2 Klein 4B distilled image-edit pipeline end-to-end.

    Loads the Flux.2 Klein 4B distilled UNet, Qwen 3 4B text encoder (flux2
    type), and Flux 2 VAE, encodes the reference image and text prompt, builds
    reference conditioning, creates an empty Flux 2 latent, runs the ``euler``
    sampler with ``Flux2Scheduler`` and ``CFGGuider`` at ``cfg=1``, and decodes
    the result.

    Negative conditioning is derived by zeroing out the positive conditioning
    via ``conditioning_zero_out`` (distilled guidance-free approach).
    ...
    cfg : float, optional
        CFG guidance scale.  Default ``1.0`` (distilled — guidance-free).
    seed : int, optional
