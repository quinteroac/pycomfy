"""Flux.2 Klein 4B distilled text-to-image pipeline.
...
This pipeline mirrors the *Text to Image (Flux.2 Klein 4B Distilled)* subgraph
in ``comfyui_official_workflows/image/generation/flux_klein/
...
_UNET_DEST = Path("diffusion_models") / "flux-2-klein-4b.safetensors"
...
def manifest() -> list[ModelEntry]:
    """Return the list of model files required by the Flux.2 Klein 4B distilled T2I pipeline.

    Returns exactly 3 :class:`~comfy_diffusion.downloader.HFModelEntry`
    instances:

    - ``diffusion_models/flux-2-klein-4b.safetensors`` — Flux.2 Klein 4B distilled UNet
...
def run(
    *,
    ...
    """Run the Flux.2 Klein 4B distilled text-to-image pipeline end-to-end.

    Loads the Flux.2 Klein 4B distilled UNet, Qwen 3 4B text encoder (flux2
    type), and Flux 2 VAE, encodes the prompt, creates an empty Flux 2 latent,
    runs the ``euler`` sampler with ``Flux2Scheduler`` and ``CFGGuider`` at
    ``cfg=1``, and decodes the result to a PIL image.
    ...
    cfg : float, optional
        CFG guidance scale.  Default ``1.0`` (distilled — effectively
        guidance-free).
    seed : int, optional
