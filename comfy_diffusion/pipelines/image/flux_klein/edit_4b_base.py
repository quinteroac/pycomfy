"""Flux.2 Klein 4B base image-edit pipeline.
...
This pipeline mirrors the *Image Edit (Flux.2 Klein 4B)* subgraph in
...
_UNET_URL = (
    ...
    "/resolve/main/flux-2-klein-base-4b-fp8.safetensors"
)

# Relative destination paths (resolved against models_dir by download_models).
_UNET_DEST = Path("diffusion_models") / "flux-2-klein-base-4b-fp8.safetensors"
...
def manifest() -> list[ModelEntry]:
    """Return the list of model files required by the Flux.2 Klein 4B base edit pipeline.

    Returns exactly 3 model entries:

    - ``diffusion_models/flux-2-klein-base-4b-fp8.safetensors`` — UNet
...
def run(
    *,
    ...
    """Run the Flux.2 Klein 4B base image-edit pipeline end-to-end.
