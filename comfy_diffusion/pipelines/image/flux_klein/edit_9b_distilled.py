"""Flux.2 Klein 9B distilled image-edit pipeline.
...
This pipeline mirrors the *Image Edit (Flux.2 Klein 9B Distilled)* subgraph in
...
_UNET_URL = (
    ...
    "/resolve/main/flux-2-klein-9b-fp8.safetensors"
)

# Relative destination paths (resolved against models_dir by download_models).
_UNET_DEST = Path("diffusion_models") / "flux-2-klein-9b-fp8.safetensors"
...
def manifest() -> list[ModelEntry]:
    """Return the list of model files required by the Flux.2 Klein 9B distilled edit pipeline.

    Returns exactly 3 model entries:

    - ``diffusion_models/flux-2-klein-9b-fp8.safetensors`` — UNet (distilled)
...
def run(
    *,
    ...
    """Run the Flux.2 Klein 9B distilled image-edit pipeline end-to-end.

    Loads the Flux.2 Klein 9B distilled UNet, Qwen 3 8B text encoder (flux2
