...
def create_video(
    ...
    if async_mode:
        ...
        run_async(action="create", media="video", model=model,
                  args={"prompt": prompt, "input": input, "width": width, "height": height,
                        "length": length, "fps": fps, "steps": steps, "cfg": cfg, "seed": seed,
                        "output": output, "models_dir": models_dir})
        return
    ...
    try:
        frames = video.RUNNERS[model](mdir=mdir, prompt=prompt, image=image_obj, w=w, h=h, n=n, f=f, s=s, c=c, seed=seed)
