#!/usr/bin/env python3
"""Flux.2 Klein 4B base text-to-image example using comfy_diffusion.

Demonstrates usage of the
``comfy_diffusion.pipelines.image.flux_klein.t2i_4b_base`` pipeline: download
models once, then call ``run()`` to generate an image from a text prompt.

The pipeline uses the Flux.2 Klein 4B base diffusion model with the Qwen 3 4B
text encoder (flux2 type), the ``euler`` sampler with ``Flux2Scheduler``, and
CFG guidance of 5.0.  It produces high-quality images at 1024×1024 resolution
in 20 steps.
...
::
    ...
    uv run python examples/flux_klein_t2i_4b_base.py --download-only

    # Generate an image
    uv run python examples/flux_klein_t2i_4b_base.py --prompt "a photo of a cat on a chair"

    # Full options
    uv run python examples/flux_klein_t2i_4b_base.py \\
        --models-dir /path/to/models \\
        --prompt "a photo of a cat sitting on a chair, photorealistic" \\
        --width 1024 --height 1024 \\
        --steps 20 --seed 42 \\
        --output flux_klein_base_output.png
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Flux.2 Klein 4B base text-to-image example (comfy_diffusion).",
    )
    parser.add_argument(
        "--models-dir",
    ...
    parser.add_argument(
        "--output",
        default="flux_klein_base_output.png",
