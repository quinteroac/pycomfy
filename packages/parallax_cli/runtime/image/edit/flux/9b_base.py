#!/usr/bin/env python3
"""Flux.2 Klein 9B base image-edit example using comfy_diffusion.

Demonstrates usage of the
``comfy_diffusion.pipelines.image.flux_klein.edit_9b_base`` pipeline:
download models once, then call ``run()`` to edit an image with a text prompt.

The 9B base variant uses the larger Qwen 3 8B text encoder for richer
prompt understanding, with CFG guidance of 5.0 and 20 denoising steps.
...
::
    ...
    uv run python examples/flux_klein_edit_9b_base.py --download-only

    # Edit an image
    uv run python examples/flux_klein_edit_9b_base.py \\
        --prompt "change the camera angle to a first-person perspective" \\
        --image photo.jpg
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Flux.2 Klein 9B base image-edit example (comfy_diffusion).",
    )
    parser.add_argument(
        "--models-dir",
    ...
    parser.add_argument(
        "--output",
        default="flux_klein_edit_9b_base_output.png",
