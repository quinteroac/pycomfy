#!/usr/bin/env python3
"""Flux.2 Klein 4B base image-edit example using comfy_diffusion.

Demonstrates usage of the
``comfy_diffusion.pipelines.image.flux_klein.edit_4b_base`` pipeline:
...
CFG guidance of 5.0.  It encodes the reference image as a conditioning latent
and runs 20 steps of editing.
...
::
    ...
    uv run python examples/flux_klein_edit_4b_base.py --download-only

    # Edit an image
    uv run python examples/flux_klein_edit_4b_base.py \\
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Flux.2 Klein 4B base image-edit example (comfy_diffusion).",
    )
    parser.add_argument(
        "--models-dir",
    ...
    parser.add_argument(
        "--prompt",
        default="change the background to a cozy indoor space with warm lighting",
    ...
    parser.add_argument(
        "--output",
        default="flux_klein_edit_4b_base_output.png",
    ...
    from comfy_diffusion.pipelines.image.flux_klein.edit_4b_base import manifest, run
