#!/usr/bin/env python3
"""
LTX-Video 2 Canny-to-Video example using comfy_diffusion pipelines.

Extracts frames from an input video, runs Canny edge detection, and generates
a new video that follows the detected edges — including a matching audio track.
Uses the 19B dev fp8 checkpoint with a Canny control LoRA.

  uv sync --extra comfyui
  uv run python examples/video/ltx/ltx2/canny.py \\
      --models-dir /path/to/models \\
      --video input.mp4 \\
      --prompt "a squirrel nibbles on a nut in a sunlit garden, gentle ambient sound"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="LTX-Video 2 Canny-to-Video example (comfy_diffusion).",
    )
    parser.add_argument(
        "--models-dir",
    ...
    parser.add_argument("--canny-lora-strength", type=float, default=1.0, help="Canny control LoRA strength (default 1.0).")
    parser.add_argument("--lora-strength", type=float, default=1.0, help="Distilled LoRA strength (default 1.0).")
    parser.add_argument("--output", default="ltx2_canny_output.mp4", help="Output video path.")
    ...
    parser.add_argument("--canny-lora-filename", default=None, help="Override Canny control LoRA filename in loras/.")
    ...
    if not args.models_dir or not Path(args.models_dir).is_dir():
        print(
            ...
            "usage: python examples/video/ltx/ltx2/canny.py --models-dir /path/to/models "
    ...
    result = run(
        ...
        seed=args.seed,
        canny_lora_strength=args.canny_lora_strength,
        ...
        canny_lora_filename=args.canny_lora_filename,
