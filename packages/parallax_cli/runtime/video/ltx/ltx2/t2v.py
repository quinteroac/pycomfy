#!/usr/bin/env python3
"""
LTX-Video 2 text-to-video (T2V) example using comfy_diffusion pipelines.

Uses the 19B dev fp8 model with distilled LoRA — generates video and audio
simultaneously via the audio-visual (AV) sampling chain.

  uv sync --extra comfyui
  uv run python examples/ltx2_t2v_example.py \\
...
def _mux_audio(video_path: str, audio_path: str, output_path: str, fps: float) -> None:
    cmd = [
        "ffmpeg", "-y",
        "-i", video_path,
        "-i", audio_path,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="LTX-Video 2 text-to-video (T2V) example (comfy_diffusion).",
    )
    parser.add_argument(
        "--models-dir",
    ...
    parser.add_argument("--steps", type=int, default=20, help="Sampling steps for pass 1 (default 20).")
    parser.add_argument("--cfg-pass1", type=float, default=4.0, help="CFG scale for pass 1 (default 4.0).")
    parser.add_argument("--cfg-pass2", type=float, default=1.0, help="CFG scale for pass 2 refinement (default 1.0).")
    parser.add_argument("--seed", type=int, default=0, help="Random seed (default 0).")
    parser.add_argument("--output", default="ltx2_t2v_output.mp4", help="Output video path.")
    ...
    parser.add_argument("--audio-vae-filename", default=None, help="Override audio VAE filename in vae/ (when separate from video VAE).")
    ...
    try:
        ...
        _mux_audio(str(tmp_video), str(tmp_audio), args.output, args.fps)
