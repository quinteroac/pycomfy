#!/usr/bin/env python3
"""
WAN 2.1 first-last-frame-to-video (FLF2V) example using comfy_diffusion pipelines.

Generates a video that interpolates between a start image and an end image using
the WAN 2.1 FLF2V 720p 14B model.  Downloads model weights automatically on the
first run (idempotent).

  uv sync --extra comfyui
  uv run python examples/video_wan21_flf2v.py \\
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="WAN 2.1 first-last-frame-to-video (FLF2V) example (comfy_diffusion).",
    )
    parser.add_argument(
        "--models-dir",
    ...
    parser.add_argument(
        "--start-image",
        required=True,
        help="Path to the start image (first frame) for interpolation.",
    )
    parser.add_argument(
        "--end-image",
        required=True,
        help="Path to the end image (last frame) for interpolation.",
    )
    parser.add_argument(
        "--prompt",
        default="glass flower blossom",
    ...
    parser.add_argument("--height", type=int, default=1280, help="Output height in px (default 1280).")
    ...
    parser.add_argument("--cfg", type=float, default=3.0, help="CFG guidance scale (default 3.0).")
    parser.add_argument("--seed", type=int, default=0, help="Random seed (default 0).")
    parser.add_argument("--output", default="wan21_flf2v_output.mp4", help="Output video path (default wan21_flf2v_output.mp4).")
    ...
    if not start_path.is_file():
        print(f"error: --start-image file not found: {args.start_image}", file=sys.stderr)
        return 1

    end_path = Path(args.end_image)
    if not end_path.is_file():
        print(f"error: --end-image file not found: {args.end_image}", file=sys.stderr)
        return 1
