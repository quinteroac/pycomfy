#!/usr/bin/env python3
"""Flux.2 Klein 4B distilled image-edit example using comfy_diffusion.

Demonstrates usage of the
``comfy_diffusion.pipelines.image.flux_klein.edit_4b_distilled`` pipeline:
download models once, then call ``run()`` to edit an image with a text prompt.

The distilled variant uses ``cfg=1`` with negative conditioning derived via
``conditioning_zero_out``, requiring only 4 steps for high-quality edits.
...
::
    ...
    uv run python examples/flux_klein_edit_4b_distilled.py --download-only

    # Edit an image
    uv run python examples/flux_klein_edit_4b_distilled.py \\
        --prompt "change the clothing to a red dress" \\
        --image photo.jpg
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Flux.2 Klein 4B distilled image-edit example (comfy_diffusion).",
    )
    parser.add_argument(
        "--models-dir",
    ...
    parser.add_argument(
        "--prompt",
        default="change the clothing to a stylish modern outfit",
    ...
    parser.add_argument(
        "--output",
        default="flux_klein_edit_4b_distilled_output.png",
