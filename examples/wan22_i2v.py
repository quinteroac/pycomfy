#!/usr/bin/env python3
"""WAN 2.2 image-to-video example using comfy_diffusion.

Demonstrates usage of the ``comfy_diffusion.pipelines.video.wan.wan22.i2v``
pipeline: download models once, then call ``run()`` to animate an input image
using the WAN 2.2 14B I2V model.

The pipeline uses the WAN 2.2 14B dual-model architecture with a dual
two-pass KSamplerAdvanced flow.  With ComfySwitchNode switches at their
default value ``False`` (no LoRA branch):

- Pass 1: low-noise UNet, add_noise=True, steps 0→steps//2
- Pass 2: high-noise UNet, add_noise=False, steps//2→steps
...
::
    ...
    uv run python examples/wan22_i2v.py --download-only

    # Animate an image
    uv run python examples/wan22_i2v.py --image input.jpg --prompt "a dragon flying through clouds"

    # Full options
    uv run python examples/wan22_i2v.py \\
        --models-dir /path/to/models \\
        --image input.jpg \\
        --prompt "a dragon flying through clouds, cinematic" \\
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="WAN 2.2 image-to-video example (comfy_diffusion).",
    )
    parser.add_argument(
        "--models-dir",
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Path to the input image to animate.",
    )
    parser.add_argument(
        "--prompt",
        default="a dragon flying through clouds, cinematic motion",
    ...
    parser.add_argument("--output", default="wan22_i2v_output.mp4", help="Output video path.")
    parser.add_argument(
        "--download-only",
        action="store_true",
    ...
    # Generate frames.
    print(f"Animating {image_path.name} → {args.length} frames ({args.width}x{args.height}) …")
    frames = run(
        input_image,
        args.prompt,
