#!/usr/bin/env python3
"""
LTX-Video 2.3 text-to-video (T2V) example using comfy_diffusion pipelines.

Uses the 22B dev fp8 model with distilled LoRA — generates video and audio
simultaneously via a two-pass sampling chain with spatial upscaling.

  uv sync --extra comfyui
  uv run python examples/ltx23_t2v_example.py \\
      --models-dir /path/to/models \\
      --prompt "a pianist playing jazz in a warm studio, soft keys and room ambience"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="LTX-Video 2.3 text-to-video (T2V) example (comfy_diffusion).",
    )
    parser.add_argument(
        "--models-dir",
    ...
    parser.add_argument(
        "--prompt",
        default="a pianist playing jazz in a warm studio, soft keys and room ambience",
    ...
    parser.add_argument("--output", default="ltx23_t2v_output.mp4", help="Output video path.")
