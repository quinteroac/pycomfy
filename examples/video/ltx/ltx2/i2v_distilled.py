#!/usr/bin/env python3
"""
LTX-Video 2 distilled image-to-video (I2V) example using comfy_diffusion pipelines.

Uses the 19B distilled checkpoint directly (no LoRA) — faster inference with
fewer steps. Animates a starting image and generates matching audio.

  uv sync --extra comfyui
  uv run python examples/ltx2_i2v_distilled_example.py \\
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="LTX-Video 2 distilled image-to-video example (comfy_diffusion).",
    )
    parser.add_argument(
        "--models-dir",
    ...
    parser.add_argument("--output", default="ltx2_i2v_distilled_output.mp4", help="Output video path.")
    parser.add_argument("--text-encoder-filename", default=None, help="Override text encoder filename in text_encoders/.")
    parser.add_argument("--unet-filename", default=None, help="Override UNet/checkpoint filename.")
