#!/usr/bin/env python3
"""
LTX-Video 2 image-to-video (I2V) example using comfy_diffusion pipelines.

Uses the 19B dev fp8 model with distilled LoRA — animates a starting image
and generates matching audio via the AV sampling chain.

  uv sync --extra comfyui
  uv run python examples/ltx2_i2v_example.py \\
      --models-dir /path/to/models \\
      --image first_frame.png \\
      --prompt "the waitress smiles and turns her head, gentle ambient sound"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="LTX-Video 2 image-to-video (I2V) example (comfy_diffusion).",
    )
    parser.add_argument(
        "--models-dir",
    ...
    parser.add_argument("--output", default="ltx2_i2v_output.mp4", help="Output video path.")
