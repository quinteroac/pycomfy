"""Tests for US-001 — comfy_diffusion/pipelines/ltx2_t2v_distilled.py pipeline.

Covers:
  AC01: manifest() returns 3 HFModelEntry items with correct dest paths
  AC02: run() accepts models_dir, prompt, negative_prompt, width, height, etc.
  AC03: pipeline calls ltxv_latent_upsample() after sampling and before vae_decode_batch_tiled()
...
# ---------------------------------------------------------------------------
# AC03 + AC04 — pipeline order (ltxv_latent_upsample before vae_decode_batch_tiled)
#               and CPU test with mocked ModelManager
# ---------------------------------------------------------------------------
...
# Lazy imports inside run() mean we must patch at source module locations.
...
def test_run_passes_upsampled_samples_to_vae_decode(
    ...
    def fake_ltxv_latent_upsample(
        ...
        assert samples is original_samples, "ltxv_latent_upsample must receive output of sample()"
    ...
    assert received_samples[0] is upsampled_samples, (
        "vae_decode_batch_tiled must receive the upsampled latent, not the original"
    )
