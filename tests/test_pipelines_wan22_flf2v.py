"""Tests for comfy_diffusion/pipelines/video/wan/wan22/flf2v.py — WAN 2.2 FLF2V pipeline.
...
Covers:
  ...
  - No bypassed-node entries (no LoRA entries)
  - run() signature: start_image, end_image, prompt, negative_prompt, width, height, length,
    *, models_dir, seed, steps, cfg
  - run() behaviour: WanFirstLastFrameToVideo conditioning followed by dual two-pass
    KSamplerAdvanced (high-noise first, low-noise second) with ModelSamplingSD3
    shift=8 applied; no LoRA
  - run() raises RuntimeError when check_runtime returns an error
  - wan22 sub-package exports flf2v
"""
...
def test_pipeline_file_exists() -> None:
    assert _PIPELINE_FILE.is_file(), "flf2v.py must exist under wan22/"
...
def test_import_manifest_and_run() -> None:
    from comfy_diffusion.pipelines.video.wan.wan22.flf2v import manifest, run  # noqa: F401
...
# ---------------------------------------------------------------------------
# Manifest checks — exactly 4 HFModelEntry items (AC-01, AC-04)
# ---------------------------------------------------------------------------


def test_manifest_returns_exactly_four_entries() -> None:
    from comfy_diffusion.pipelines.video.wan.wan22.flf2v import manifest
...
def test_manifest_text_encoder_dest_path() -> None:
    """AC-02: active CLIP loader (umt5_xxl) is included in manifest()."""
    from comfy_diffusion.pipelines.video.wan.wan22.flf2v import manifest
...
def test_manifest_no_lora_entries() -> None:
    """AC-04: bypassed LoRA nodes must not appear in manifest()."""
    from comfy_diffusion.pipelines.video.wan.wan22.flf2v import manifest

    dests = [str(e.dest) for e in manifest()]
    assert not any("loras" in d for d in dests), (
        "manifest() must not include lora entries (all LoRA nodes are bypassed)"
    )
...
# ---------------------------------------------------------------------------
# run() signature checks (AC-03)
# ---------------------------------------------------------------------------


def test_run_signature_includes_required_params() -> None:
    from comfy_diffusion.pipelines.video.wan.wan22.flf2v import run

    sig = inspect.signature(run)
    required = {
        "start_image", "end_image", "prompt", "negative_prompt",
        "width", "height", "length",
        "models_dir", "seed", "steps", "cfg",
    }
...
def test_run_default_cfg_4() -> None:
    from comfy_diffusion.pipelines.video.wan.wan22.flf2v import run
...
def _run_with_mocks(
    ...
    *,
    ...
    from comfy_diffusion.pipelines.video.wan.wan22 import flf2v as pipeline_mod

    if fake_frames is None:
        fake_frames = [MagicMock(name="frame0")]

    fake_flf2v_latent = MagicMock(name="flf2v_latent")
    ...
    fake_start_image = MagicMock(name="pil_start")
    fake_end_image = MagicMock(name="pil_end")
    image_to_tensor_count: list[int] = [0]

    def fake_image_to_tensor(img: Any) -> Any:
        image_to_tensor_count[0] += 1
        name = f"image_to_tensor_{image_to_tensor_count[0]}"
        _track(name, None)
    ...
    with (
        ...
        patch(_IMAGE_TO_TENSOR_PATCH, side_effect=fake_image_to_tensor),
        patch(_WAN_FLF2V_PATCH, side_effect=lambda pos, neg, vae, **kw: (
            _track("wan_first_last_frame_to_video", (pos, neg, fake_flf2v_latent))
        )),
        ...
        return pipeline_mod.run(
            fake_start_image,
            fake_end_image,
            "a crystal cat transforming into a giant beast",
            models_dir=tmp_path,
        )


# ---------------------------------------------------------------------------
# run() behaviour tests (AC-03)
# ---------------------------------------------------------------------------
...
def test_run_calls_wan_first_last_frame_to_video(tmp_path: Path) -> None:
    """WanFirstLastFrameToVideo must be called to build FLF2V conditioning."""
    call_order: list[str] = []
    _run_with_mocks(tmp_path, call_order=call_order)
    assert "wan_first_last_frame_to_video" in call_order


def test_run_calls_image_to_tensor_twice(tmp_path: Path) -> None:
    """Both start and end images must be converted to tensors."""
    call_order: list[str] = []
    _run_with_mocks(tmp_path, call_order=call_order)
    tensor_calls = [c for c in call_order if c.startswith("image_to_tensor_")]
    assert len(tensor_calls) == 2, (
        f"image_to_tensor must be called twice (start + end), got {tensor_calls}"
    )


def test_run_does_not_call_apply_lora(tmp_path: Path) -> None:
    """No LoRA should be applied — all LoRA nodes are bypassed."""
    from comfy_diffusion.pipelines.video.wan.wan22 import flf2v as pipeline_mod

    fake_start = MagicMock(name="pil_start")
    fake_end = MagicMock(name="pil_end")
    mm = _build_mock_mm()
    ...
    with (
        ...
        patch(_WAN_FLF2V_PATCH, return_value=(MagicMock(), MagicMock(), MagicMock())),
        ...
        pipeline_mod.run(fake_start, fake_end, "test prompt", models_dir=tmp_path)
...
def test_run_calls_model_sampling_sd3_twice(tmp_path: Path) -> None:
    ...
    assert len(sd3_calls) == 2, (
        f"model_sampling_sd3 must be called twice, got {sd3_calls}"
    )
...
def test_run_calls_sample_advanced_twice(tmp_path: Path) -> None:
    ...
    assert len(sa_calls) == 2, (
        f"sample_advanced must be called twice, got {sa_calls}"
    )
...
def test_run_pass1_add_noise_true(tmp_path: Path) -> None:
    """First KSamplerAdvanced pass (high-noise) must have add_noise=True."""
    from comfy_diffusion.pipelines.video.wan.wan22 import flf2v as pipeline_mod
    ...
    with (
        ...
        pipeline_mod.run(
            MagicMock(), MagicMock(), "test prompt", models_dir=tmp_path
        )
...
def test_run_pass2_add_noise_false(tmp_path: Path) -> None:
    """Second KSamplerAdvanced pass (low-noise) must have add_noise=False."""
    from comfy_diffusion.pipelines.video.wan.wan22 import flf2v as pipeline_mod
...
def test_run_model_sampling_sd3_shift_is_8(tmp_path: Path) -> None:
    """ModelSamplingSD3 must be called with shift=8.0 for both models."""
    from comfy_diffusion.pipelines.video.wan.wan22 import flf2v as pipeline_mod
    ...
    assert all(s == 8.0 for s in captured_shifts), (
        f"model_sampling_sd3 must be called with shift=8.0 for both models, got {captured_shifts}"
    )
...
def test_wan22_exports_flf2v() -> None:
    import comfy_diffusion.pipelines.video.wan.wan22 as wan22_pkg

    assert "flf2v" in wan22_pkg.__all__, (
        f"wan22/__init__.py __all__ must contain 'flf2v', got {wan22_pkg.__all__!r}"
    )


def test_wan22_flf2v_module_importable() -> None:
    import importlib

    mod = importlib.import_module("comfy_diffusion.pipelines.video.wan.wan22.flf2v")
    assert callable(mod.manifest)
    assert callable(mod.run)


# ---------------------------------------------------------------------------
# download_models idempotent — 4 entries (AC-01)
# ---------------------------------------------------------------------------
