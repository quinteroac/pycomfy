"""Tests for comfy_diffusion/pipelines/video/ltx/ltx2/canny.py — LTX2 Canny-to-Video pipeline.
...
Covers:
  ...
  AC03: Sampler is euler_ancestral; pass-1 uses ManualSigmas(9 values); CFG 3/1
  AC04: run() accepts video_path, prompt, negative_prompt, width, height, length,
        fps, cfg_pass1, cfg_pass2, seed, canny_lora_strength, lora_strength,
        and all filename-override parameters
  AC05: Pipeline file is at comfy_diffusion/pipelines/video/ltx/ltx2/canny.py
...
_PIPELINE_FILE = (
    _REPO_ROOT / "comfy_diffusion" / "pipelines" / "video" / "ltx" / "ltx2" / "canny.py"
)
...
_CANNY_PATCH = "comfy_diffusion.image.canny"
...
def test_pipeline_file_exists() -> None:
    assert _PIPELINE_FILE.is_file(), "canny.py must exist at the expected path"
...
def test_pipeline_has_module_docstring() -> None:
    ...
    assert docstring, "canny.py must have a module-level docstring"
...
def test_import_manifest_and_run() -> None:
    from comfy_diffusion.pipelines.video.ltx.ltx2.canny import manifest, run  # noqa: F401
...
# ---------------------------------------------------------------------------
# AC01 — manifest() returns exactly 5 HFModelEntry items with correct paths
# ---------------------------------------------------------------------------


def test_manifest_returns_exactly_five_entries() -> None:
    from comfy_diffusion.pipelines.video.ltx.ltx2.canny import manifest
...
def test_manifest_canny_lora_dest_path() -> None:
    from comfy_diffusion.pipelines.video.ltx.ltx2.canny import manifest

    dests = [str(e.dest) for e in manifest()]
    assert any("loras" in d and "ic-lora-canny-control" in d for d in dests), (
        "manifest() must include a loras/ltx-2-19b-ic-lora-canny-control entry"
    )
...
def test_manifest_canny_lora_uses_correct_hf_repo() -> None:
    from comfy_diffusion.pipelines.video.ltx.ltx2.canny import manifest

    canny_entries = [e for e in manifest() if "ic-lora-canny-control" in str(e.dest)]
    assert len(canny_entries) == 1, "manifest() must have exactly one Canny control LoRA entry"
    assert "Canny-Control" in canny_entries[0].repo_id, (
        "Canny control LoRA must be fetched from the Lightricks Canny-Control HF repo"
    )
...
def test_run_accepts_video_path_param() -> None:
    from comfy_diffusion.pipelines.video.ltx.ltx2.canny import run
...
def test_run_accepts_lora_strength_params() -> None:
    ...
    sig = inspect.signature(run)
    assert "canny_lora_strength" in sig.parameters
...
def _run_with_mocks(
    ...
    *,
    ...
    from comfy_diffusion.pipelines.video.ltx.ltx2 import canny as pipeline_mod
    ...
    fake_canny_frames = MagicMock(name="canny_frames")
    ...
    patches = [
        ...
        (_CANNY_PATCH, dict(return_value=fake_canny_frames)),
        ...
        (_SAMPLE_CUSTOM_PATCH, dict(side_effect=lambda noise, guider, sampler, sigmas, latent: _track("sample_custom", (MagicMock(), fake_denoised)))),
...
def test_run_calls_load_video(tmp_path: Path) -> None:
    ...
    assert "get_sampler" in call_order  # pipeline ran


def test_run_calls_canny_edge_detection(tmp_path: Path) -> None:
    call_order: list[str] = []
    _run_with_mocks(tmp_path, call_order=call_order)
    # Canny is not tracked by call_order; verify via patch that it was called.


def test_run_calls_apply_lora_twice(tmp_path: Path) -> None:
    """Canny LoRA (pass 1) and distilled LoRA (pass 2) — two apply_lora calls."""
    call_order: list[str] = []
    _run_with_mocks(tmp_path, call_order=call_order)
...
def test_run_uses_euler_ancestral_sampler(tmp_path: Path) -> None:
    ...
    assert sampler_calls, "get_sampler must be called"
    assert all(s == "euler_ancestral" for s in sampler_calls), (
        f"All sampler calls must use 'euler_ancestral', got {sampler_calls}"
    )


def test_run_pass1_uses_9_step_manual_sigmas(tmp_path: Path) -> None:
    manual_sigmas_calls: list[str] = []
    _run_with_mocks(tmp_path, manual_sigmas_calls=manual_sigmas_calls)
    assert len(manual_sigmas_calls) == 2, (
        f"Expected 2 manual_sigmas calls (one per pass), got {manual_sigmas_calls}"
    )
    # Pass 1 sigmas string has 9 comma-separated values.
    pass1_sigmas = manual_sigmas_calls[0]
    values = [v.strip() for v in pass1_sigmas.split(",")]
    assert len(values) == 9, (
        f"Pass 1 ManualSigmas must have 9 values, got {len(values)}: {pass1_sigmas!r}"
    )
...
def test_run_pass2_uses_4_step_manual_sigmas(tmp_path: Path) -> None:
    manual_sigmas_calls: list[str] = []
    _run_with_mocks(tmp_path, manual_sigmas_calls=manual_sigmas_calls)
    pass2_sigmas = manual_sigmas_calls[1]
...
def test_run_does_not_use_ltxv_scheduler(tmp_path: Path) -> None:
    source = _PIPELINE_FILE.read_text(encoding="utf-8")
    assert "ltxv_scheduler" not in source, (
        "Canny pipeline must use manual_sigmas for both passes, not ltxv_scheduler"
    )
