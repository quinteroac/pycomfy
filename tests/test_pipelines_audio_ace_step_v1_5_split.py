"""Tests for comfy_diffusion/pipelines/audio/ace_step/v1_5/split.py.

Covers:
  - File created at the correct path                                           (AC01)
  - manifest() returns four HFModelEntry items                                 (AC02)
  - run() loads models with load_unet, load_clip(ace), load_vae               (AC03)
  - Sampling execution follows the same order as the checkpoint pipeline       (AC04)
...
def test_pipeline_file_exists() -> None:
    assert _PIPELINE_FILE.is_file(), (
        "split.py must exist under pipelines/audio/ace_step/v1_5/"
    )
...
def test_pipeline_has_module_docstring() -> None:
    ...
    assert docstring, "split.py must have a module-level docstring"
...
def test_import_manifest_and_run() -> None:
    from comfy_diffusion.pipelines.audio.ace_step.v1_5.split import manifest, run  # noqa: F401
...
def test_manifest_contains_1_7b_clip_entry() -> None:
    from comfy_diffusion.pipelines.audio.ace_step.v1_5.split import manifest

    filenames = [e.filename for e in manifest()]
    assert any("qwen_1.7b_ace15" in f for f in filenames), (
        f"manifest() must contain qwen_1.7b_ace15 CLIP entry; got {filenames}"
    )
...
def _run_with_mocks(
    ...
    **run_kwargs: Any,
    ...
    from comfy_diffusion.pipelines.audio.ace_step.v1_5 import split as pipeline_mod
...
def test_run_calls_load_clip_with_both_clip_files(tmp_path: Path) -> None:
    ...
    assert "qwen_1.7b_ace15" in positional_args[1]
