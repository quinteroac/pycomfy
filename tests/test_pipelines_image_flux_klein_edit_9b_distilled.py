"""CPU smoke tests for comfy_diffusion/pipelines/image/flux_klein/edit_9b_distilled.py.

Covers (US-006):
  - AC01: edit_9b_distilled.py exists at the correct path
...
def _run_with_mocks(tmp_path: Path, **run_kwargs: Any) -> list[Any]:
    from comfy_diffusion.pipelines.image.flux_klein import edit_9b_distilled as mod
...
def test_pipeline_file_exists() -> None:
    assert _PIPELINE_FILE.is_file(), "edit_9b_distilled.py must exist"
...
def test_pipeline_has_module_docstring() -> None:
    source = _PIPELINE_FILE.read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(_PIPELINE_FILE))
    assert ast.get_docstring(tree), "edit_9b_distilled.py must have a module-level docstring"
...
def test_manifest_returns_three_entries() -> None:
    from comfy_diffusion.pipelines.image.flux_klein.edit_9b_distilled import manifest
...
def test_manifest_unet_filename() -> None:
    ...
    assert any("flux-2-klein-9b-fp8.safetensors" in d for d in dest_names)
