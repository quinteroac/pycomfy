"""CPU smoke tests for comfy_diffusion/pipelines/image/flux_klein/edit_9b_base.py.

Covers (US-006):
  - AC01: edit_9b_base.py exists at the correct path
...
def _run_with_mocks(tmp_path: Path, **run_kwargs: Any) -> list[Any]:
    from comfy_diffusion.pipelines.image.flux_klein import edit_9b_base as mod
...
def test_pipeline_file_exists() -> None:
    assert _PIPELINE_FILE.is_file(), "edit_9b_base.py must exist"
...
def test_pipeline_has_module_docstring() -> None:
    source = _PIPELINE_FILE.read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(_PIPELINE_FILE))
    assert ast.get_docstring(tree), "edit_9b_base.py must have a module-level docstring"
...
def test_manifest_returns_three_entries() -> None:
    from comfy_diffusion.pipelines.image.flux_klein.edit_9b_base import manifest
...
def test_manifest_unet_filename() -> None:
    ...
    assert any("flux-2-klein-base-9b-fp8.safetensors" in d for d in dest_names)
...
def test_run_calls_vae_encode(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(_SAMPLE_CUSTOM_PATCH, return_value=(fake_latent_out, MagicMock())),
        patch(_VAE_DECODE_PATCH, return_value=MagicMock(name="image")),
        patch(_VAE_ENCODE_PATCH, return_value={"samples": MagicMock()}) as mock_encode,
    ):
...
# ===========================================================================
# AC05: default cfg=5, steps=20 (base)
# ===========================================================================
