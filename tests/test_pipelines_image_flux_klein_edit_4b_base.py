"""CPU smoke tests for comfy_diffusion/pipelines/image/flux_klein/edit_4b_base.py.

Covers (US-006):
  - AC01: edit_4b_base.py exists at the correct path
  - AC02: manifest() returns exactly 3 entries with correct filenames/dirs
  - AC03: run() follows node order (vae_encode → reference_latent × 2,
          EmptyFlux2LatentImage, Flux2Scheduler, CFGGuider, SamplerCustomAdvanced,
          VAEDecode) and returns list[PIL.Image]
  - AC05: Base variant defaults cfg=5, steps=20
...
def _run_with_mocks(tmp_path: Path, **run_kwargs: Any) -> list[Any]:
    from comfy_diffusion.pipelines.image.flux_klein import edit_4b_base as mod
...
def test_pipeline_file_exists() -> None:
    assert _PIPELINE_FILE.is_file(), "edit_4b_base.py must exist"
...
def test_pipeline_has_module_docstring() -> None:
    source = _PIPELINE_FILE.read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(_PIPELINE_FILE))
    assert ast.get_docstring(tree), "edit_4b_base.py must have a module-level docstring"
...
def test_manifest_returns_three_entries() -> None:
    from comfy_diffusion.pipelines.image.flux_klein.edit_4b_base import manifest
...
def test_manifest_unet_filename() -> None:
    ...
    assert any("flux-2-klein-base-4b-fp8.safetensors" in d for d in dest_names)
...
def test_run_result_is_image(tmp_path: Path) -> None:
    with patch(_RUNTIME_PATCH, return_value=_OK_RUNTIME):
        result = _run_with_mocks(tmp_path)
...
# ===========================================================================
# AC03: node execution order — key calls are made
# ===========================================================================


def test_run_calls_vae_encode(tmp_path: Path) -> None:
    from comfy_diffusion.pipelines.image.flux_klein import edit_4b_base as mod
    ...
    with (
        ...
        patch(_SAMPLE_CUSTOM_PATCH, return_value=(fake_latent_out, MagicMock())),
        patch(_VAE_DECODE_PATCH, return_value=MagicMock(name="image")),
        patch(_VAE_ENCODE_PATCH, return_value={"samples": MagicMock()}) as mock_vae_encode,
    ):
        mod.run(
            models_dir=tmp_path,
            prompt="test",
...
def test_run_calls_reference_latent(tmp_path: Path) -> None:
    from comfy_diffusion.pipelines.image.flux_klein import edit_4b_base as mod
    ...
    with (
        ...
        patch(_MM_PATCH, return_value=mm),
        patch(_ENCODE_PATCH, return_value=(MagicMock(), MagicMock())),
        patch(_REFERENCE_LATENT_PATCH, return_value=MagicMock()) as mock_ref_latent,
        patch(_EMPTY_FLUX2_LATENT_PATCH, return_value={"samples": MagicMock()}),
        patch(_RANDOM_NOISE_PATCH, return_value=MagicMock()),
        patch(_GET_SAMPLER_PATCH, return_value=MagicMock()),
        patch(_FLUX2_SCHEDULER_PATCH, return_value=MagicMock()),
    ...
    # reference_latent called twice: once for positive, once for negative
    assert mock_ref_latent.call_count == 2


# ===========================================================================
# AC05: default cfg=5, steps=20
# ===========================================================================


def test_default_cfg_is_5(tmp_path: Path) -> None:
    from comfy_diffusion.pipelines.image.flux_klein import edit_4b_base as mod
    ...
    cfg_val = mock_cfg.call_args[0][3] if mock_cfg.call_args[0] else kwargs.get("cfg")
    assert cfg_val == 5.0


def test_default_steps_is_20(tmp_path: Path) -> None:
    from comfy_diffusion.pipelines.image.flux_klein import edit_4b_base as mod
