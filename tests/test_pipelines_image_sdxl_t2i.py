"""CPU smoke tests for comfy_diffusion/pipelines/image/sdxl/t2i.py.
...
_PIPELINE_FILE = (
    _REPO_ROOT / "comfy_diffusion" / "pipelines" / "image" / "sdxl" / "t2i.py"
)
...
def test_pipeline_file_exists() -> None:
    assert _PIPELINE_FILE.is_file(), "sdxl/t2i.py must exist"
...
def test_pipeline_has_module_docstring() -> None:
    source = _PIPELINE_FILE.read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(_PIPELINE_FILE))
    assert ast.get_docstring(tree), "sdxl/t2i.py must have a module-level docstring"
...
def test_manifest_base_dest_path() -> None:
    ...
    assert any("checkpoints" in d and "sd_xl_base" in d for d in dests), (
        "manifest() must include a checkpoints/sd_xl_base_1.0.safetensors entry"
    )
...
def test_manifest_refiner_dest_path() -> None:
    ...
    assert any("checkpoints" in d and "sd_xl_refiner" in d for d in dests), (
        "manifest() must include a checkpoints/sd_xl_refiner_1.0.safetensors entry"
    )


def test_manifest_uses_stabilityai_repos() -> None:
    from comfy_diffusion.pipelines.image.sdxl.t2i import manifest

    repos = [entry.repo_id for entry in manifest()]
    assert any("stabilityai/stable-diffusion-xl-base" in r for r in repos)
    assert any("stabilityai/stable-diffusion-xl-refiner" in r for r in repos)
...
def test_run_signature_includes_required_params() -> None:
    ...
    assert required <= set(sig.parameters.keys()), (
        f"run() missing parameters: {required - set(sig.parameters.keys())}"
    )
...
def _run_with_mocks(tmp_path: Path, **run_kwargs: Any) -> list[Any]:
    ...
    with (
        ...
        patch(_SAMPLE_ADVANCED_PATCH, return_value=fake_latent_out),
...
def test_run_loads_two_checkpoints(tmp_path: Path) -> None:
    ...
    assert mm.load_checkpoint.call_count == 2, (
        "run() must load exactly 2 checkpoints (base + refiner)"
    )
