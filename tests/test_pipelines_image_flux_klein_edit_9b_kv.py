"""CPU smoke tests for comfy_diffusion/pipelines/image/flux_klein/edit_9b_kv.py.

Covers (US-006):
  - AC01: edit_9b_kv.py exists at the correct path
  ...
  - AC04: run() calls flux_kv_cache, image_scale_to_total_pixels, accepts two images
  - AC05: Defaults cfg=1, steps=4
...
_FLUX_KV_CACHE_PATCH = "comfy_diffusion.sampling.flux_kv_cache"
...
_IMAGE_SCALE_PATCH = "comfy_diffusion.image.image_scale_to_total_pixels"
...
def _run_with_mocks(tmp_path: Path, **run_kwargs: Any) -> list[Any]:
    from comfy_diffusion.pipelines.image.flux_klein import edit_9b_kv as mod
    ...
    with (
        ...
        patch(_FLUX_KV_CACHE_PATCH, return_value=MagicMock(name="kv_model")),
        ...
        patch(_VAE_ENCODE_TENSOR_PATCH, return_value={"samples": MagicMock()}),
        patch(_IMAGE_SCALE_PATCH, return_value=fake_scaled),
        patch(_IMAGE_TO_TENSOR_PATCH, return_value=MagicMock(name="tensor")),
        patch(_GET_IMAGE_SIZE_PATCH, return_value=(1024, 1024)),
    ):
        return mod.run(
            models_dir=tmp_path,
            prompt="test prompt",
            reference_image=MagicMock(name="ref_image"),
            subject_image=MagicMock(name="subj_image"),
            **run_kwargs,
        )
...
def test_pipeline_file_exists() -> None:
    assert _PIPELINE_FILE.is_file(), "edit_9b_kv.py must exist"
...
def test_pipeline_has_module_docstring() -> None:
    source = _PIPELINE_FILE.read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(_PIPELINE_FILE))
    assert ast.get_docstring(tree), "edit_9b_kv.py must have a module-level docstring"
...
def test_manifest_returns_three_entries() -> None:
    from comfy_diffusion.pipelines.image.flux_klein.edit_9b_kv import manifest
...
def test_manifest_unet_filename() -> None:
    ...
    assert any("flux-2-klein-9b-kv-fp8.safetensors" in d for d in dest_names)
...
def test_manifest_clip_filename() -> None:
    ...
    entries = manifest()
    dest_names = [str(e.dest) for e in entries]
    assert any("qwen_3_8b_fp8mixed.safetensors" in d for d in dest_names)
...
def test_run_result_is_not_none(tmp_path: Path) -> None:
    with patch(_RUNTIME_PATCH, return_value=_OK_RUNTIME):
        result = _run_with_mocks(tmp_path)
...
# ===========================================================================
# AC04: flux_kv_cache called, image_scale_to_total_pixels called, two images
# ===========================================================================


def test_run_calls_flux_kv_cache(tmp_path: Path) -> None:
    from comfy_diffusion.pipelines.image.flux_klein import edit_9b_kv as mod

    mm = _build_mock_mm()
    fake_latent_out = MagicMock(name="latent_out")
    fake_scaled = MagicMock(name="scaled")
    ...
    with (
        ...
        patch(_GET_SAMPLER_PATCH, return_value=MagicMock()),
        patch(_FLUX2_SCHEDULER_PATCH, return_value=MagicMock()),
        patch(_FLUX_KV_CACHE_PATCH, return_value=MagicMock()) as mock_kv,
        patch(_CFG_GUIDER_PATCH, return_value=MagicMock()),
        patch(_SAMPLE_CUSTOM_PATCH, return_value=(fake_latent_out, MagicMock())),
        patch(_VAE_DECODE_PATCH, return_value=MagicMock(name="image")),
        patch(_VAE_ENCODE_TENSOR_PATCH, return_value={"samples": MagicMock()}),
        ...
        mod.run(
            ...
            subject_image=MagicMock(),
        )
    mock_kv.assert_called_once()


def test_run_calls_image_scale_to_total_pixels_twice(tmp_path: Path) -> None:
    from comfy_diffusion.pipelines.image.flux_klein import edit_9b_kv as mod
    ...
    with (
        ...
        patch(_GET_SAMPLER_PATCH, return_value=MagicMock()),
        patch(_FLUX2_SCHEDULER_PATCH, return_value=MagicMock()),
        patch(_FLUX_KV_CACHE_PATCH, return_value=MagicMock()),
        patch(_CFG_GUIDER_PATCH, return_value=MagicMock()),
        patch(_SAMPLE_CUSTOM_PATCH, return_value=(fake_latent_out, MagicMock())),
        patch(_VAE_DECODE_PATCH, return_value=MagicMock(name="image")),
        patch(_VAE_ENCODE_TENSOR_PATCH, return_value={"samples": MagicMock()}),
        patch(_IMAGE_SCALE_PATCH, return_value=fake_scaled) as mock_scale,
        patch(_IMAGE_TO_TENSOR_PATCH, return_value=MagicMock()),
        patch(_GET_IMAGE_SIZE_PATCH, return_value=(1024, 1024)),
    ):
        mod.run(
...
def test_run_calls_reference_latent_four_times(tmp_path: Path) -> None:
    from comfy_diffusion.pipelines.image.flux_klein import edit_9b_kv as mod
    ...
    # 4 reference_latent calls: pos+ref, pos+subj, neg+ref, neg+subj
    assert mock_ref.call_count == 4


# ===========================================================================
# AC05: default cfg=1, steps=4
# ===========================================================================
