"""Tests for US-003 — comfy_diffusion/pipelines/ltx2_i2v_distilled.py pipeline (AV chain).

Covers:
  AC01: manifest() returns 3 HFModelEntry items with correct dest paths
  AC02: run() mirrors ltx2_i2v.run() signature but uses distilled checkpoint
  AC03: default steps=8
  AC04: no apply_lora() call in the pipeline
  AC05: CPU test passes with mocked inputs; AV chain executed in correct order
...
def test_pipeline_file_exists() -> None:
    assert _PIPELINE_FILE.is_file(), (
        "comfy_diffusion/pipelines/ltx2_i2v_distilled.py must exist"
    )
...
def test_manifest_unet_dest_path_uses_distilled_checkpoint() -> None:
    ...
    assert any("diffusion_models" in d and "ltx-2-19b-distilled" in d for d in dests), (
        "manifest() must include an entry with dest under diffusion_models/ using distilled checkpoint"
    )
...
def test_manifest_no_lora_entry() -> None:
    ...
    entries = manifest()
    dests = [str(e.dest) for e in entries]
    assert not any("lora" in d for d in dests), (
        "manifest() must not include a LoRA entry for the distilled pipeline"
    )
...
# ---------------------------------------------------------------------------
# AC02 — run() mirrors ltx2_i2v.run() signature (same parameters)
# ---------------------------------------------------------------------------
...
def test_run_signature_includes_image_parameter() -> None:
    ...
    required = {"models_dir", "image", "prompt", "negative_prompt", "width", "height"}
...
def test_run_signature_matches_ltx2_i2v() -> None:
    ...
    # Distilled has no lora_strength or lora_filename; remove those from i2v for comparison.
    lora_params = {"lora_strength", "lora_filename"}
    i2v_non_lora = i2v_params - lora_params

    assert i2v_non_lora == distilled_params, (
        f"run() signature mismatch (excluding LoRA params).\n"
        f"  Missing from distilled: {i2v_non_lora - distilled_params}\n"
        f"  Extra in distilled:     {distilled_params - i2v_non_lora}"
    )


def test_run_uses_distilled_checkpoint_not_dev_fp8() -> None:
    source = _PIPELINE_FILE.read_text(encoding="utf-8")
    assert "ltx-2-19b-distilled" in source
    assert "ltx-2-19b-dev-fp8" not in source
...
def test_run_calls_pipeline_steps_in_order(tmp_path: Path) -> None:
    """Pipeline executes AV chain in the correct order."""
    call_order: list[str] = []
    result = _run_with_mocks(tmp_path, _make_fake_image(), call_order=call_order)
    ...
    assert call_order == expected, (
        f"Expected {expected}, got {call_order}"
    )
    assert isinstance(result, dict)
...
def test_run_uses_load_ltxav_text_encoder_not_load_clip(tmp_path: Path) -> None:
    ...
    with (
        ...
        pipeline_mod.run(models_dir=tmp_path, image=_make_fake_image(), prompt="test")
