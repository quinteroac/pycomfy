"""Tests for AuraFlow model sampling helper."""
...
from comfy_diffusion.models import model_sampling_aura_flow
...
def _install_fake_aura_flow_sampling_modules(
    monkeypatch: Any,
) -> dict[str, list[Any]]:
    ...
    class FakeModelSamplingDiscreteFlow:
        ...
        def set_parameters(self, *, shift: float, multiplier: float) -> None:
...
def test_model_sampling_aura_flow_is_public_and_callable_from_models_module() -> None:
    assert callable(model_sampling_aura_flow)
    assert model_sampling_aura_flow.__name__ == "model_sampling_aura_flow"
    assert "model_sampling_aura_flow" in models_module.__all__


def test_model_sampling_aura_flow_signature_matches_contract() -> None:
    signature = inspect.signature(model_sampling_aura_flow)
    assert str(signature) == "(model: 'Any', shift: 'float') -> 'Any'"


def test_model_sampling_aura_flow_returns_patched_clone_and_applies_aura_flow_sampling(
    monkeypatch: Any,
) -> None:
    model_config = object()
    model = _FakeModel(model_config=model_config)
    calls = _install_fake_aura_flow_sampling_modules(monkeypatch)

    patched = model_sampling_aura_flow(model, shift=1.73)
    ...
    assert calls["set_parameters"] == [{"shift": 1.73, "multiplier": 1.0}]


def test_model_sampling_aura_flow_shift_controls_continuous_v_prediction_schedule(
    monkeypatch: Any,
) -> None:
    model = _FakeModel(model_config=object())
    calls = _install_fake_aura_flow_sampling_modules(monkeypatch)

    model_sampling_aura_flow(model, shift=1.2)
    model_sampling_aura_flow(model, shift=2.8)

    assert calls["set_parameters"] == [
        {"shift": 1.2, "multiplier": 1.0},
        {"shift": 2.8, "multiplier": 1.0},
    ]
    assert calls["set_parameters"][0]["shift"] != calls["set_parameters"][1]["shift"]


def test_import_model_sampling_aura_flow_has_no_heavy_import_side_effects() -> None:
    result = _run_python(
        "import json\n"
        "import sys\n"
        ...
        "payload = {\n"
        "  'func_name': model_sampling_aura_flow.__name__,\n"
    ...
    assert payload["func_name"] == "model_sampling_aura_flow"
